...
class JT808Manager:
    ...
    def record_attendance_event(self, phone_num: str, event_type: str, lat: float | None, lon: float | None) -> dict:
        ...
        try:
            ...
            saved = attendance_service.record_device_event(phone_num, event_type, lat, lon)


            event["attendance_id"] = saved.get("_id")
            ...
            event["status"] = "save_failed"


            event["save_error"] = str(exc)


            logger.error(f"[打卡] 写入 attendance_record 失败: {exc}")
