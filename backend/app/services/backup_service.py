...
BackupService.create_full_backup = _create_mongo_backup
BackupService.restore_mysql_backup = _restore_mongo_backup
def _restore_mongo_backup_result(self, filename: str) -> Dict:
    success = self.restore_mysql_backup(filename)
    return {"success": success, "message": "MongoDB restore completed" if success else "MongoDB restore failed"}


BackupService.restore_backup = _restore_mongo_backup_result
