...
from app.core.database import get_mongo_db, get_mongo_collection, get_next_sequence
...
# 杩戝疄鏃跺洖鏀句緷璧栫煭鍒嗘;甯告€佸洖鏀剧敱鐙珛褰掓。閫昏緫瀹屾垚,涓嶄笌鍒嗘鏃堕暱缁戝畾?
...
<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:tptz="http://www.onvif.org/ver20/ptz/wsdl">
  ...
  <s:Body>
    ...
    def get_stream_info(self, mongo_db, video_id: int):
        db_video = self._get_video_runtime_by_id(video_id)
    ...
    def add_camera_to_media_server(self, mongo_db, camera_data: CameraCreateRequest):
    ...
    def create_video(self, mongo_db, video_data: VideoCreate):
        collection = self._video_collection()
    ...
    def get_videos(self, mongo_db, skip: int = 0, limit: int = 100):
        collection = self._video_collection()
    ...
    def save_playback_clip(self, video_id: int, start_time: datetime | str, end_time: datetime | str,
        ...
        try:
            ...
            if output_type == "alarm":
                ...
                mirror_subdir = os.path.join("playback_videos", "temp_cache")

                primary_root = self._get_temp_playback_root()

            else:

                mirror_subdir = "playback_videos"
