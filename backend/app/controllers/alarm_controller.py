...
def create_alarm(
    ...
    if alarm.project_id is None:
        ...
        if user_project_id not in [None, ""]:
            alarm.project_id = user_project_id
...
@router.post("/test")
def create_test_alarm(payload: AlarmTestRequest, current_user: dict = Depends(get_current_user)):
    ...
    alarm_doc = service._apply_org_snapshot_to_payload(alarm_doc)
...
def update_alarm(
    ...
    updated = service.update_alarm(db, alarm_id, alarm, current_user=current_user)
    if not updated:
...
def delete_alarm(
    ...
    success = service.delete_alarm(db, alarm_id, current_user=current_user)
    if not success:
        raise HTTPException(status_code=404, detail="Alarm not found")
