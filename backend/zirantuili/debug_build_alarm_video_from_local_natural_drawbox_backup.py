...
pipeline. It uses a local alarm snapshot as the only trusted time/bbox anchor,
cuts overlapping local recording MP4s, draws a temporary red box around the
snapshot second, and writes raw/boxed debug videos plus metadata JSON.
"""
...
def build_snapshot_candidate(path: Path, ffprobe: Path | None, ffmpeg: Path) -> SnapshotCandidate:
    notes: list[str] = []
...
def select_batch_candidates(
    ...
    usable = [item for item in candidates if item.bbox_norm]
...
def build_alarm_video_artifacts(
    ...
    boxed_duration = probe_duration(ffprobe, ffmpeg, output_boxed)
    ...
    meta = {
        ...
        "boxed_duration": round(boxed_duration, 3),
        ...
        "box_start_second": round(box_start, 3),
        "box_end_second": round(box_end, 3),
...
def run_batch(args: argparse.Namespace) -> int:
    ...
    for path in snapshot_paths:
        try:
            candidate = build_snapshot_candidate(path, ffprobe, ffmpeg)
        except Exception as exc:
            skipped.append({"snapshot_image": str(path), "reason": str(exc), "notes": []})
            continue
        if not candidate.bbox_norm:
            skipped.append({"snapshot_image": str(path), "reason": "bbox not found", "notes": candidate.notes})
            continue
        candidates.append(candidate)

    selected, relaxed = select_batch_candidates(candidates, range_start, range_end, int(args.max_snapshots))
    ...
    for candidate in selected:
        ...
        if candidate.bbox_original_pixel:
...
def run_single(args: argparse.Namespace) -> int:
    ...
    log(f"boxed video: {output_boxed}")
