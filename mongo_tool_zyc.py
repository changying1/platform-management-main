...
# 默认操作的数据库名称
DEFAULT_DB_NAME = "smart_helmet_mongo"
# 备份存放目录
BACKUP_DIR = "MongoData"
# 备份文件名。优先使用完整全库导出文件；不存在时回退到旧备份文件。
BACKUP_FILENAME = "smart_helmet_mongo_full_export.json"
LEGACY_BACKUP_FILENAME = "all_collections_backup.json"
# ==========================================

BACKUP_PATH = os.path.join(BACKUP_DIR, BACKUP_FILENAME)
LEGACY_BACKUP_PATH = os.path.join(BACKUP_DIR, LEGACY_BACKUP_FILENAME)

def get_restore_backup_path():
    if os.path.exists(BACKUP_PATH):
        return BACKUP_PATH
    return LEGACY_BACKUP_PATH

def normalize_backup_data(raw_data):
    """
    支持两种格式：
    1. 旧格式：{ "collection_name": [docs...] }
    2. 完整格式：{ "database": "...", "collections": { "collection_name": [docs...] } }
    """
    if (
        isinstance(raw_data, dict)
        and isinstance(raw_data.get("collections"), dict)
        and "database" in raw_data
    ):
        return raw_data["collections"]
    return raw_data
...
def restore_database(db_name):
    ...
    """
    restore_path = get_restore_backup_path()
    if not os.path.exists(restore_path):
        print(f"错误: 备份文件 '{restore_path}' 不存在，请先执行备份操作。")
        return
    ...
    try:
        with open(restore_path, 'r', encoding='utf-8') as f:
            ...
            all_data = normalize_backup_data(json_util.loads(file_content))
