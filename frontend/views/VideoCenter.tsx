...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const pollKeyboardSwitchRequest = async () => {
      ...
      const requestedVideoId = Number(request.video_id);
      const requestedVideoText = String(request.video_id).trim();
      const target = devices.find((device) => (
        Number(device.id) === requestedVideoId ||
        String(device.name || "").trim() === requestedVideoText
      ));
      ...
      try {
        const switched = await handleShowStream(target);
        if (!switched) {
          return;
        }
        ...
        console.warn(`Keyboard switch failed: video_id=${request.video_id}`, err);
      }
    };
