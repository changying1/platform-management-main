...
def finish_device_id_input():
    with device_id_input_lock:
        if (
            ...
            log(f"DEVICE_SWITCH finish ignored: input timeout, buffer={device_id_input_state.buffer!r}")
            device_id_input_state.active = False
            device_id_input_state.buffer = ""
            device_id_input_state.pending_video_id = None
            device_id_input_state.updated_at = time.time()
            return
        if not device_id_input_state.active:
            log("DEVICE_SWITCH finish ignored: not in input mode")
            return
    ...
    if not raw:
        log("DEVICE_SWITCH finish ignored: empty video_id")
        return

    try:
        video_id = int(raw)
    except ValueError:
        log(f"DEVICE_SWITCH finish failed: invalid video_id={raw!r}")
        return

    if video_id <= 0:
        log(f"DEVICE_SWITCH finish failed: video_id must be positive, got={video_id}")
        return

    with device_id_input_lock:
        device_id_input_state.pending_video_id = video_id
        device_id_input_state.updated_at = time.time()

    log(f"DEVICE_SWITCH input finished pending_video_id={video_id}")


def confirm_device_switch():
    with device_id_input_lock:
        video_id = device_id_input_state.pending_video_id
        device_id_input_state.pending_video_id = None
        device_id_input_state.active = False
        device_id_input_state.buffer = ""
        device_id_input_state.updated_at = time.time()
    ...
    if not video_id:
        log("DEVICE_SWITCH confirm ignored: no pending video_id")
        return
...
def handle_device_switch_preset_action(action: str, preset_no: int):
    ...
    if action == "preset_clear":
        confirm_device_switch()
        return

    log(f"unknown device switch preset action: {action}")
